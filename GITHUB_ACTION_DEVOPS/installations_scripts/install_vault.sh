...
vault write auth/approle/role/github-actions-role token_num_uses=0 id_num_uses=0 policies="github-actions-policy"

ROLE_ID=`vault read auth/approle/role/github-actions-role/role-id | grep role_id | awk '{print $2}'`
SECRET_ID=`vault write -f auth/approle/role/github-actions-role/secret-id | awk '$1 == "secret_id" { print $2 }'`
...
echo "Use the role ID and secret ID to authenticate the github-actions workflow with Vault." >> vaultkey.txt
