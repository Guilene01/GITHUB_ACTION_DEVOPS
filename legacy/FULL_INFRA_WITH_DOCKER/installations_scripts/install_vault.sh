...
vault write auth/approle/role/tools-role token_num_uses=0 id_num_uses=0 policies="tools"

ROLE_ID=`vault read auth/approle/role/tools-role/role-id | grep role_id | awk '{print $2}'`
SECRET_ID=`vault write -f auth/approle/role/tools-role/secret-id  | awk '$1 == "secret_id" { print $2 }'`

vault secrets enable -path=secrets kv

cat > tools-policy.hcl << EOF
path "secrets/creds/*" {
 capabilities = ["read"]
}
EOF

vault policy write tools tools-policy.hcl
...
echo "Use the role ID and secret ID to authenticate the tools workflow with Vault." >> vaultkey.txt
