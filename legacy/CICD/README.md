...
This folder contains an older Jenkins-based CI/CD deployment that is no longer aligned with the current repository architecture.

## Current repo focus
The active deployment path has moved to:
- `GITHUB_ACTION_DEVOPS/` for GitHub Actions, JFrog, Vault, Trivy, and SonarCloud
- `FULL_INFRA_WITH_DOCKER/` for a secondary JFrog + Vault tools stack

## Legacy status
This module is kept for historical reference only. It is not maintained and still contains Jenkins/SonarQube-specific Terraform and scripts.

If you are looking for the current implementation, use the root `README.md` and the `GITHUB_ACTION_DEVOPS/` module.
